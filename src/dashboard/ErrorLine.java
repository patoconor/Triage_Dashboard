...
public class ErrorLine {
	...
	public static ArrayList<ListItem> insertLongRun(ArrayList<ListItem> listitems2){
		...
		try {
            String connectionURL = "jdbc:jtds:sqlserver://L4DWIPDS011.hewitt.com:13163/;" + "databaseName=FileConfig;user=WMTriage;password=WM_0xC#Octal103;";
            ...
            while(resultSet.next()){
            	System.out.println(resultSet.getString("fileID"));
            }
			} catch (Exception e) {
	            e.printStackTrace();
	        } finally {
	...
	public static ArrayList<ListItem> insertMessageQ(ArrayList<ListItem> listitems2){
		...
		try {
            ...
            for(int i=0;i<listitems2.size();i++){
            	System.out.println(listitems2.get(i).getFileID()+": "+listitems2.get(i).getErrorMessage());
            }
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
	...
	public void createErrorLine(String fistUsername, String fistPassword, ArrayList<ListItem> list, WebDriver dr) throws InterruptedException
	{
			...
			try {
	            ...
	            connection = DriverManager
	                    .getConnection(
	                            ...
	                            "sysdba", "masterkey");
	            statement = connection.createStatement();
	            resultSet2 = statement.executeQuery("select DISTINCT * from TRIAGE where ERDATE = '"+dateFormat.format(date1)+"'");
	            ...
	            while(resultSet2.next()){
	            	...
	            	String textMinus = resultSet2.getString("FILEID").trim()+"|"+resultSet2.getString("ERDATE").trim()+"|"+resultSet2.getString("ERTIME").trim()+"|"+resultSet2.getString("SERVER").trim()+resultSet2.getString("FIST").trim()+"|"+resultSet2.getString("ERROR");
	            	
	            	//System.out.println(text);
	            	String text1 = fileID.trim()+"|"+date.trim()+"|"+time.trim()+"|"+server.trim()+"|"+FullLine.trim()+"|"+errorMessage;
	            	System.out.println("ErrorMessageMatch: "+errorMessage.equals(resultSet2.getString("ERROR")));
	            	if(textMinus.equals(text1)){
	            		insert = false;
	            		System.out.println("Match found, no insert.");
	            	}
	            ...
	            if(insert == true && partial == true){
	            	System.out.println("Partial match, inserting");
	...
	private void populateFieldsInFIST(WebDriver dr) throws InterruptedException
	{
		clearExpectations();
